...
const template = ({
  ...
  gulp.task("template", () => {
    ...
    let gulpStaticStreamCollection = templateCollection.map((folderName) => {
      ...
      let dest = path.join(taskTarget, folderName);
    ...
    let gulpStreamCollection = templateCollection.map((folderName) => {
      ...
      let templateData =
        getJsonData({ dataPath: path.join(dir.source, "_" + folderName) }) ||
        {};
      ...
      return Object.keys(templateData)
        ...
        .map((value) => {
          return (
            gulp
              .src(path.join(dir.source, "_" + folderName, "template.pug"))
              // .pipe(plugins.debug())
              // compile pug to html
              .pipe(
                plugins.pug({
              ...
              .pipe(
                plugins.rename(
                  config.render.url.htmlExtensionOn
                    ? `${value}.html`
                    : `${value}/index.html`
                )
              )
